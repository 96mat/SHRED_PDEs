...
# %%
dataset = WellDataset(
    well_base_path="./datasets/datasets",
...
# %%
print(len(dataset))
item = dataset[320] #number of the dataset
...
# %%
manager = DataManager(
    lags=1001, #       # 1 year of weekly history as input
...
manager.add_data(
    ...
    id="GSRD",          # Unique identifier for the dataset
...
LSTM_Custom = LSTM_Forecaster(
    lags=100,          # numero di step temporali di memoria
...
train_Y = {"GSRD": data[0:t_train]}  # Ground truth segment
train_error = engine.evaluate(manager.train_sensor_measurements, train_Y)

# ---------------- Validation Evaluation ----------------
t_val = len(manager.val_sensor_measurements)
val_Y = {"GSRD": data[t_train:t_train + t_val]}
val_error = engine.evaluate(manager.val_sensor_measurements, val_Y)

# ---------------- Test Evaluation ----------------
t_test = len(manager.test_sensor_measurements)
test_Y = {"GSRD": data[-t_test:]}
...
# %%
compare_data = [truth[:,:,0], reconstruction[:,:,0], forecast[:,:,0]]
...
for ax, d, title in zip(axes, compare_data, titles):
    im = ax.imshow(d, vmin=vmin, vmax=vmax, cmap='turbo')
    ax.set_title(title)
...
for i in range(len(manager.sensor_summary_df)):
    ax.scatter(pos[i][1],pos[i][0])
# Add shared colorbar
fig.colorbar(im, ax=axes, orientation='vertical', label="GSRD", \
             shrink=0.8,)
plt.show()
# %%
compare_data = [truth[:,:,1], reconstruction[:,:,1], forecast[:,:,1]]
titles = ["Test Ground Truth", "SHRED Reconstruction", "SHRED Forecast"]
